...
def run_at_specific_time(hour, minute, client: AimHarderClient, target_day: str, class_id1, class_id2):
    ...
    """
    target_time = datetime.now().replace(hour=hour, minute=minute+3, second=0, microsecond=0)
    # target_time = datetime.now() + timedelta(minutes=1)
    # if target_time < datetime.now():
    #     target_time += timedelta(days=1)

    if (int(target_time.strftime("%M")) - int(datetime.now().strftime("%M"))) > 5:
        raise Exception("Más de 5 minutos de diferencia", target_time.strftime("%H:%M:%S"))
...
def main(email="your.email@mail.com", password="1234", box_name="lahuellacrossfit", box_id="3984", days_in_advance="2"):
    ...
    run_at_specific_time(15, 20, client, target_day, class_id1, class_id2)
    # client.book_class(target_day, class_id1)
    # client.book_class(target_day, class_id2)
